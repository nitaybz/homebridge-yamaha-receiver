		...
		RemoteKey: function(key, callback) {
			...
			switch (key) {
				...
				case RemoteKey.PLAY_PAUSE:
					this.log(`${this.name} - Sending Remote Key: "PLAY/PAUSE" (toggling)`)
					this.avr.pause().catch(() => this.avr.play().catch(() => {}))
					break;
				case RemoteKey.FAST_FORWARD:
				case RemoteKey.NEXT_TRACK:
					this.log(`${this.name} - Sending Remote Key: "SKIP"`)
					this.avr.skip().catch(() => {})
					break;
				case RemoteKey.REWIND:
				case RemoteKey.PREVIOUS_TRACK:
					this.log(`${this.name} - Sending Remote Key: "REWIND"`)
					this.avr.rewind().catch(() => {})
					break;
				default:
			}
			callback()
