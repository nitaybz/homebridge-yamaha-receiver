...
const createNewConfig = async (config, avr, availableInputs, log) => {
	...
	try {
		const newConfig = {
			...
			model: config.model,
			zone1: {
				name: await getZoneName(avr, 'Main_Zone', log),
		...
		for (const i of [2,3,4]) {
			if (config.features[`Zone_${i}`] && config.features[`Zone_${i}`][0] === '1') {
				...
				newConfig[`zone${i}`] = {
					...
					name: await getZoneName(avr, `Zone_${i}`, log),
					inputs: mapInputs(availableInputs),
...
const getZoneName = async function(avr, zone, log) {
	return avr.getZoneConfig(zone)
		.then(config => {
			log.easyDebug(`Got ${zone} config:`)
			log.easyDebug(JSON.stringify(config))
